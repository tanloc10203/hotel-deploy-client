...
// * Sign UP HOTEL
function* fetchSignUpHotel({ payload }) {
  try {
    const response = yield call(authAPI.signUpHotel, payload);

    if (response) {
      yield put(authActions.signUpHotelSucceed());
      yield put(appActions.setOpenOverlay(false));
      yield put(appActions.setText(""));
      history.push(`/verify-otp/${response.data.user_id}`);
    }
  } catch (error) {
    yield put(appActions.setOpenOverlay(false));
    yield put(appActions.setText(""));
...
function* watchFetchSignUpHotel() {
  yield takeLatest(authActions.signUpHotelStart.type, fetchSignUpHotel);
}
